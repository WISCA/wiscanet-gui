...
struct Context<'a, 'b>{ msg: Option<(&'a str, &'b str)>, edgenodes: Vec<Edgenode> }

impl<'a, 'b> Context<'a, 'b> {
    pub fn err(conn: &DbConn, msg: &'a str) -> Context<'static, 'a> {
        Context{msg: Some(("error", msg)), edgenodes: Edgenode::all(conn)}
    }

    pub fn raw(conn: &DbConn, msg: Option<(&'a str, &'b str)>) -> Context<'a, 'b> {
        Context{msg: msg, edgenodes: Edgenode::all(conn)}
    }
}
...
fn delete(id: i32, conn: DbConn) -> Result<Flash<Redirect>, Template> {
    if Edgenode::delete_with_id(id, &conn) {
        Ok(Flash::success(Redirect::to("/"), "Node was deleted."))
    } else {
        Err(Template::render("index", &Context::err(&conn, "Couldn't delete edgenode.")))
    }
}
